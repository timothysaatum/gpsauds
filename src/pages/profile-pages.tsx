  ...
  return (
    <div className="section-container py-12 max-w-2xl">
      ...
      <div className="card p-6 mb-6 flex items-center gap-5">
        ...
        <div>
          <p className="font-700 text-lg text-[#1c2b22]">{user.full_name}</p>
          <p className="text-sm text-[#5a7060]">{user.email}</p>
      ...
      <div className="card p-8">
        <h2 className="font-body font-700 text-[#1c2b22] mb-6">Edit Profile</h2>
        ...
        <form onSubmit={handleSubmit((d) => mutation.mutate(d))} className="space-y-4">
          ...
          <div>
            ...
            <p className="form-error text-[#5a7060]">Email cannot be changed.</p>
          </div>
        ...
        <div className="space-y-4">
          {certs.map((cert) => (
            <div key={cert.id} className="card p-6 flex items-center gap-5">
              <div className="w-12 h-12 rounded-2xl bg-gold-50 flex items-center justify-center text-2xl flex-shrink-0">🏅</div>
              <div className="flex-1">
                <p className="font-700 text-[#1c2b22]">Certificate of Participation</p>
                <p className="text-sm text-[#5a7060]">Issued {formatDate(cert.issued_at)}</p>
                <p className="text-xs text-[#5a7060] font-mono mt-0.5">{cert.verification_code}</p>
              </div>
              {cert.download_url && (
...
export function CertificateVerifyPage() {
  ...
  return (
    <div className="section-container py-16 max-w-lg text-center">
      ...
      <p className="text-[#5a7060] mb-8">Enter a certificate verification code to confirm its authenticity.</p>
...
export function SettingsPage() {
  ...
  return (
    <div className="section-container py-12 max-w-xl space-y-8">
      ...
      {/* Change password */}
      <div className="card p-8">
        <h2 className="font-body font-700 text-[#1c2b22] text-lg mb-5">Change Password</h2>
        {pwDone && (
          <div className="mb-4 flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl p-3.5 text-sm text-green-700">
      ...
      {/* Security */}
      <div className="card p-8">
        <h2 className="font-body font-700 text-[#1c2b22] text-lg mb-2">Active Sessions</h2>
        <p className="text-sm text-[#5a7060] mb-5">Sign out from all other devices for security.</p>
        <Button
